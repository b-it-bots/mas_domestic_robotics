...
class GetGuestInfo(smach.State):
    ...
    def __init__(self,
        ...
        # VLM service for saving guest appearance image (optional)
        self.vlm_service = None
        self._bridge = CvBridge() if CV_AVAILABLE else None
        try:
            from hsr_task_sm.srv import VLMQuery
            rospy.wait_for_service('/vlm/query', timeout=3.0)
            self.vlm_service = rospy.ServiceProxy('/vlm/query', VLMQuery)
            rospy.loginfo('[GetGuestInfo] VLM service connected — will save guest images.')
            if self.guest_number == 1:
                self.vlm_service(query_type='clear_faces', image_base64='', context='')
                rospy.loginfo('[GetGuestInfo] Cleared previous guest images for new run.')
        except Exception:
            rospy.logwarn('[GetGuestInfo] VLM service not available — skipping image save.')
    ...
    def _save_guest_image(self, name):
        """Capture head camera image and save to VLM server for later recognition."""
...
class RecognizePerson(smach.State):
    ...
    def __init__(self,
        ...
        try:
            from hsr_task_sm.srv import VLMQuery
            rospy.wait_for_service('/vlm/query', timeout=5.0)
            self.vlm_service = rospy.ServiceProxy('/vlm/query', VLMQuery)
            rospy.loginfo('[RecognizePerson] VLM service connected.')
        except Exception as e:
            rospy.logwarn('[RecognizePerson] VLM service not available: %s', e)
