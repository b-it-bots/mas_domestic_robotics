...
class WaitForDoor(smach.State):
    def __init__(self, sleep_duration=1., door_status_topic='/mcr_perception/door_status/door_status'):
        ...
        self.door_status_sub = rospy.Subscriber(door_status_topic, Bool, self.update_door_status)
...
class EnterDoor(smach.State):
    def __init__(self, timeout=120.0, move_forward_server='/mdr_actions/move_forward_server', movement_duration=15., speed=0.1):
        smach.State.__init__(self, outcomes=['succeeded', 'failed'])
        self.timeout = timeout
        self.movement_duration = movement_duration
        self.speed = speed

        self.move_forward_client = actionlib.SimpleActionClient(move_forward_server, MoveForwardAction)
    ...
    def execute(self, userdata):
        ...
        self.move_forward_client.wait_for_result(rospy.Duration.from_sec(self.timeout))
