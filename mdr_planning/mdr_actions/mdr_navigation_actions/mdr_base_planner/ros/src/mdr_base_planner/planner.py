...
class t2d2t3d:
    ...
    def plan_grab_poses(self, cloud, transform_3D2head, transform_head2map, base_pose_robot, bbox, viz=False):
        try:
            #bbox = [[x1,y1],[x2,y2]]
        ...
        valid_points_head = transform_3D2head(valid_points_3D)
        real_object_head = transform_3D2head([obj_pose])[0]
        valid_points_map = transform_head2map(valid_points_head)
        real_object_pose = transform_head2map([real_object_head])[0]
