...
class PerceivePlanes(ScenarioStateBase):
    def __init__(self, save_sm_state=False, **kwargs):
        ...
        self.plane_say = kwargs.get('plane_say', 0)
        ...
        self.retry_count = 0
        self.cat = None
        self.head_config_joints = dict(kwargs.get('head_joints', dict()))
        self.arm_config_joints = dict(kwargs.get('arm_joints', dict()))
    ...
    #         userdata.object_tilt=self.tilt_angle
        ...
        rospy.loginfo('Perceiving %s' % self.plane_say)
        self.say('Perceiving ' + self.plane_say)
        ...
        if self.succeeded:
            self.say('%s perceived' % self.plane_say)
            rospy.loginfo('%s perceived successfully' % self.plane_say)

            userdata.shelf = self.cat
            
            return 'succeeded'

        rospy.loginfo('Could not perceive %s' % self.plane_say)
        self.say('Could not perceive ' + self.plane_say)
        if self.retry_count == self.number_of_retries:
            rospy.loginfo('Failed to perceive %s' % self.plane_say)
            self.say('Aborting operation')
            return 'failed_after_retrying'
        rospy.loginfo('Retrying to perceive %s' % self.plane_say)
        self.retry_count += 1
        return 'failed'
