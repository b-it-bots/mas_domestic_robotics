...
class PickClosestFromSurface(ScenarioStateBase):
    ...
    def execute(self, userdata):
        ...
        self.retry_count = 0
        #new for heartmet23
        obj_to_grasp_instance = self.kb_interface.get_obj_instance(obj_to_grasp, Object._type) #
        self.grasping_context = self.get_grasping_strategy(obj_to_grasp_instance) 
        #new for heartmet23
    ...
    def get_grasping_strategy(self, object_to_pick_up): #
        '''The grasping position is given by the center of the
        ...
        '''        
        rospy.loginfo('xxxxxxxx %.3f , YYYYYYYYY %.3f , ZZZZZZZZ %.3f',object_to_pick_up.bounding_box.dimensions.x, object_to_pick_up.bounding_box.dimensions.y, object_to_pick_up.bounding_box.dimensions.z) # 
        rospy.loginfo('xxxxxxxx %.3f , YYYYYYYYY %.3f , ZZZZZZZZ %.3f',object_to_pick_up.dimensions.vector.x, object_to_pick_up.dimensions.vector.y, object_to_pick_up.dimensions.vector.z) #
        
        if object_to_pick_up.dimensions.vector.z > max(object_to_pick_up.dimensions.vector.x, object_to_pick_up.bounding_box.dimensions.y): #
            rospy.loginfo('[STRATEGY STRATEGY STRATEGY] TOP')
            return 'top_grasp'
            
        else: # 
            rospy.loginfo('[STRATEGY STRATEGY STRATEGY] SIDE')            
            return 'sideways_grasp'
