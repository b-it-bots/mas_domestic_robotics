...
class InteractionClient(ScenarioStateBase):
    ...
    def get_info(self, objects=None, locations=None):
        ...
        init_message = requests.post(RASA_SERVER_URL, json={"message": "scenario 1"})
        self.say_this(init_message)
        while count != 0:
            with sr.Microphone() as source:
            ...
            try:
                ...
                try:
                    ...
                    if data["text"] == "conversation_ongoing":
                        ...
                        self.say_this(data['data']['response']
                        count -= 1
...
"""
    ...
    def execute(self, userdata):
        ...
        '''if not confirmed:
            ...
            return 'failed'''
        #else:
        self.say_this("Hello, I am Lucy, here to serve you. What can i fetch for you today", time_out=3)
        retries = 2
