...
class InteractionClient(ScenarioStateBase):
    ...
    def display_image(self, image_path):
        ...
        cv2.namedWindow("Image"+str(self.q), cv2.WINDOW_NORMAL)
        cv2.setWindowProperty("Image"+str(self.q), cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_FULLSCREEN)
        cv2.imshow('Image'+str(self.q), image)
        cv2.waitKey(0)  # Display the image for 10 seconds (10,000 milliseconds)
    ...
    def get_info(self, objects=None, locations=None):
        ...
        image_thread = threading.Thread(target=self.display_image, args=("/home/lucy/ros/noetic/src/mas_domestic_robotics/mdr_planning/mdr_behaviours/mdr_hri_behaviours/disp_imgs/Slide2.PNG",))
        # Start the thread
        image_thread.start()
        ...
        if not objects or not locations:
            ...
            if not objects:
                image_thread1 = threading.Thread(target=self.display_image, args=("/home/lucy/ros/noetic/src/mas_domestic_robotics/mdr_planning/mdr_behaviours/mdr_hri_behaviours/disp_imgs/Slide1.PNG",))
                # Start the thread
                image_thread1.start()
