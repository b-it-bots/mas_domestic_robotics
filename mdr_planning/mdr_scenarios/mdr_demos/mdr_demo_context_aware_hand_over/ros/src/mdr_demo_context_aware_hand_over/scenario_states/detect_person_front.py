...
class DetectPerson(ScenarioStateBase):
    def __init__(self, save_sm_state=False, **kwargs):
        ScenarioStateBase.__init__(self, 'detect_person_front',
                                   ...
                                   output_keys=['person_list',
                                                'person_location'],
                                   input_keys=['person_location', 'head_tilt'])
    ...
    def execute(self, userdata):
        ...
        rospy.loginfo(
            '[detect_person] Sending action lib goal to {0}'.format(self.action_server))
        ...
        if result and result.person_list.persons:
            rospy.loginfo('[detect_person] Found {0} people'.format(
                len(result.person_list.persons)))
